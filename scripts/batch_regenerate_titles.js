...
// FIXED template filename to match actual file
const templatePath = path.join(__dirname, "../titles", "a-nightmare-on-elm-street-part-2-freddy-s-revenge-1985.html");
...
titleData.forEach(({ slug, title, date, rating, description }) => {
  ...
  const content = template
    ...
    .replace(/1985\s*•\s*Rating:\s*5.8/g, `${date} • Rating: ${rating}`)
    ...
    .replace(/<title>.*?<\/title>/gi, `<title>${title} - Retrograde DB</title>`);
